...
The visual canvas palette is data-driven. `New-BGInfoVisualCanvas` accepts color parameters for the hero title, subtitle, tile label/value/detail text, glass tile fill, progress track, badge fill, badge text, and primary/secondary accents. Use `-NoHeroBadge` when the central badge should be omitted entirely instead of recolored. Those colors and badge visibility are preserved in JSON exports, so a consumer can brand the same reusable ChartForgeX template without changing renderer code.
